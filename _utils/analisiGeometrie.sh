...
errori=$(sqlite3 "$cartellaDati"/analisi.sqlite "SELECT count(*) FROM regioni_check")
...
rm -r "$cartellaDati"/shp_clean/regioni/*

# esporta tabella con log errori file di input
ogr2ogr -f CSV "$cartellaDati"/shp_clean/regioni/regioni_check.csv "$cartellaDati"/analisi.sqlite regioni_check

# crea shapefile con geometrie corrette
ogr2ogr "$cartellaDati"/shp_clean/regioni/regioni.shp "$cartellaDati"/analisi.sqlite regioni_clean
